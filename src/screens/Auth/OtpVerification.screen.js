...
import { signin, setUserToken, setUserType, setUserName, setUserEmail, setStudent, setTempEmail, setTempPassword } from '@actions/authAction.js';
...
const SignupVerification = ({ navigation }) => {
	...
	const saveUserToken = async (userName, userEmail, userType, token, expiryDate, student) => {
		let userData;
		if (student) {
			userData = {
				USER_TYPE: userType,
		...
		try {
			await AsyncStorage.setItem('hostals_USER_DATA', JSON.stringify(userData));
		}
		catch (err) {
		}
		// getUserToken();
							...
							.then((res) => {
								if (res.data.err) {
									...
									setTimeout(() => {
										...
										let user = res.data;
										saveUserToken(user.userName, user.userEmail, user.userType, user.token, expiryDate, user.student);
		...
		<>
			...
			<View style={{ flexGrow: 1, height: '100%', backgroundColor: 'white' }}>
				...
				<View style={{ height: HEIGHT * 0.15, justifyContent: 'space-between' }}>
					<Text style={{ alignSelf: 'center', textAlign: 'center', fontSize: 15, }}>Didn’t received the code?  <Text style={{ color: constants.M1 }} onPress={() => { resendOTP() }}>Resend</Text></Text>
