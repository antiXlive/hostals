...
import { fetchStudents } from '@actions/fetcherAction';
import { deleteStudent } from '@actions/deleterAction';
import { CircularPLusButton } from '@components/CustomButton.component';
import { BottomSheetForm_Add_Student } from '@components/BottomSheetForm.component';
...
const ManageStudent = (props) => {
    ...
    const students = useSelector((state) => state.fetcher.students);
    ...
    const handleDelete = () => {
        ...
        dispatch(deleteStudent(data, TOKEN));
        setOP(false);
    }
    ...
    return (
        <View style={{ width: WIDTH, height: HEIGHT * 0.98, paddingTop: HEIGHT * 0.1, alignItems: 'center' }}>
            ...
            }
            {
                students && students.length > 0 ? (
                    ...
                    <>
                        ...
                        <Text style={{ alignSelf: 'center', fontSize: WIDTH * 0.06, opacity: 0.5, letterSpacing: WIDTH * 0.005 }}>No Students Found</Text>
                    </>
                ):null
            }
            ...
            {
                showForm ? (
                    <BottomSheetForm_Add_Student label='Add Student' closeModal={() => setSF(false)} token={TOKEN} />
                ) : null
            }
            {
                options ? (
